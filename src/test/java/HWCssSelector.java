...
//        System.out.println(element2.getText());
    ...
    @Test()
    public void HwXpath() {
        List<WebElement> elements1 = driver.findElements(By.xpath("//*[@class='mob-top-menu']"));// findClass
        System.out.println(elements1.size());
        WebElement element3 = driver.findElement(By.xpath("//*[@id='dialog-notifications-error']"));// findID +#
        System.out.println(element3.getAttribute("title"));
        driver.findElement(By.xpath("//div[@class='header-links-wrapper']"));//Tag+class
        driver.findElement(By.xpath("//div[@id='flyout-cart' and @class='flyout-cart']"));//Tag+Id+class
        WebElement element4 = driver.findElement(By.xpath("//img[@alt='Tricentis Demo Web Shop']"));//Tag+Attribut
        System.out.println(element4.getAttribute("alt"));
        driver.findElement(By.xpath("//form[@action='/search' and @onsubmit='return check_small_search_form()' and @novalidate='novalidate']"));//Tag+Attribut+Attribut+
        driver.findElement(By.xpath("//input[@value='Search']"));//Tag+Attribut

        WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));

        WebElement element2 = wait.until(
                ExpectedConditions.visibilityOfElementLocated(By.xpath("//li[1]")));// Tag(li)+child1
        System.out.println(element2.getText());

        //  visibilityOfElementLocated делает по сути то же самое что и implicitlyWait
        // только implicitlyWait мгновенно засчитаывает элементы в DOM он его нашел и передает управления дальше
        // видим ли он или нет его это не очень волнует
        // а visibilityOfElementLocated проверяет элемент в DOM ,и ждемт пока он загрузиться  станет видемый только
        // после этого передает управления




        WebElement element5 =driver.findElement(By.xpath("//input[@value='Search']"));
        System.out.println(element5.getAttribute("value"));
